...
if (-not $env:WINPYDIR -eq [System.IO.Path]::GetFullPath( $env:WINPYDIRBASE+"\python-3.5.2") ) {


$env:WINPYDIR = $env:WINPYDIRBASE+"\python-3.5.2"


$env:WINPYVER = '3.5.2.3Zero'
...
if (-not $env:PATH.ToLower().Contains(";"+ $env:WINPYDIR.ToLower()+ ";"))  {
 $env:PATH = "$env:WINPYDIR\Lib\site-packages\PyQt5;$env:WINPYDIR\Lib\site-packages\PyQt4;$env:WINPYDIR\;$env:WINPYDIR\DLLs;$env:WINPYDIR\Scripts;$env:WINPYDIR\..\tools;$env:WINPYDIR\..\tools\mingw32\bin;$env:WINPYDIR\..\tools\R\bin\i386;$env:WINPYDIR\..\tools\Julia\bin;$env:path;" }
